...
Udgp=preferences(dYu,ExpUt)          # All preference orders
U=preferences(dYu)
G=matrixcons(gindex, Menus, Udgp)      # Marix of 0 and 1
Omegadiag=ones(size(G,1))           # Vector of weigts
## Data on menus
menusdata=Matrix(CSV.read(rootdir*"/Codes/Julia_code_nail/newfunctions/data/menu_choice_pooled.csv", DataFrame))[:,1]
## MM probabilities
  dU=length(Udgp)
  pmm=zeros(32,6)
...
@everywhere begin
  ...
  function powerseed(seed)
    ...
    Boot=pmap(seedt->ksbootseedpower(seedt, X, ghat, etahat, taun),1:500)
    boot=collect(Boot)
    pvalue=mean(Tn.<=boot)
    return pvalue
  end
...
CSV.write(rootdir*"/Codes/Julia_code_nail/newfunctions/powerresults/power_pval_$(lambda)_$(s1)_$(s2).csv", DataFrame(Output',:auto))
println("Done")
