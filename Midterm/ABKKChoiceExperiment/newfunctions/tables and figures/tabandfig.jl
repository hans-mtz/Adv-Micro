...
dirresults=rootdir*"/Codes/Julia_code_nail/newfunctions/tables and figures/results"
pooled = Matrix(CSV.read(rootdir*"/Codes/Julia_code_nail/newfunctions/data/menu_choice_pooled.csv", DataFrame));
LC = Matrix(CSV.read(rootdir*"/Codes/Julia_code_nail/newfunctions/data/menu_choice_low.csv", DataFrame));
MC = Matrix(CSV.read(rootdir*"/Codes/Julia_code_nail/newfunctions/data/menu_choice_medium.csv", DataFrame));
HC = Matrix(CSV.read(rootdir*"/Codes/Julia_code_nail/newfunctions/data/menu_choice_high.csv", DataFrame));
...
for j = 1:3
        for i = 1:5
                ...
                r = findall(x->size(intersect(x,k))[1]!==0 ,C[j][:,1])  #Rows in Data satsifes |A|=i
                F6[i,j] = count(x->x==0, C[j][r,:][:,2])/size(r)[1]
        end
end
F6
# Draw Figure 6
xlab = ["High Cost", "Medium Cost", "Low Cost"];
Figure6 = plot(F6',xticks = (1:1:3, xlab),
        ...
        label = ["|A| = 1" "|A| = 2" "|A| = 3" "|A| = 4" "|A| = 5"],legendfontsize = 7)
...
xlab = CategoricalArray(repeat(["High Cost", "Medium Cost", "Low Cost"], outer = 6))
levels!(xlab,["High Cost", "Medium Cost", "Low Cost"])
...
Figure7 = groupedbar(xlab, F7', group = legend, ylim = (0,0.4), yticks = 0:0.05:0.4,
        title = "", bar_width = 0.7, legendfontsize=6,
...
CSV.write(dirresults*"/Table4.csv",  DataFrame(Table4), writeheader=false)
#==================================Table 5======================================#
p025=Matrix(CSV.read(rootdir*"/Codes/Julia_code_nail/newfunctions/powerresults/power_pval_0.25_1_500.csv", DataFrame));
p05=Matrix(CSV.read(rootdir*"/Codes/Julia_code_nail/newfunctions/powerresults/power_pval_0.5_1_500.csv", DataFrame));
